...
void Parser::Parse(const char *buf) {
    ...
    Clear();
    while (true) {
        auto line = FindField(start, nullptr, "\r\n");
        if (line.size() == 0) {
            break;
        }
        if (start == buf) {
            _strMethod = FindField(line.data(), nullptr, " ");
            ...
            _strTail = FindField(line.data(), (_strFullUrl + " ").data(), nullptr);
        } else {
            auto field = FindField(line.data(), nullptr, ": ");
            auto value = FindField(line.data(), ": ", nullptr);
            if (field.size() != 0) {
                _mapHeaders.emplace_force(field, value);
            }
            auto field2 = FindField(line.data(), nullptr, ":");
            auto value2 = FindField(line.data(), ":", nullptr);
            if (field2.size() != 0) {
                _mapHeaders.emplace_force(field2, value2);
            }
        }
