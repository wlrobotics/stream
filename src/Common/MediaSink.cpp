...
//如果添加Track，最多等待5秒
#define MAX_WAIT_MS_ADD_TRACK 500
...
void MediaSink::checkTrackIfReady(const Track::Ptr &track){
    ...
    if(!_all_track_ready){
        if(_ticker.elapsedTime() > MAX_WAIT_MS_READY){
            DebugL << "如果超过规定时间，那么不再等待并忽略未准备好的Track stream_id=" << stream_id_;
            emitAllTrackReady();
            return;
        }

        if(!_track_ready_callback.empty()){
            DebugL << "在超时时间内，如果存在未准备好的Track，那么继续等待 stream_id=" << stream_id_;
            return;
        }

        if(_track_map.size() == _max_track_size){
            DebugL << "如果已经添加了音视频Track，并且不存在未准备好的Track，那么说明所有Track都准备好了 stream_id=" << stream_id_;
            emitAllTrackReady();
            return;
        }

        if(_track_map.size() == 1 && _ticker.elapsedTime() > MAX_WAIT_MS_ADD_TRACK){
            DebugL << "如果只有一个Track，那么在该Track添加后，我们最多还等待若干时间(可能后面还会添加Track) stream_id=" << stream_id_;
            emitAllTrackReady();
            return;
        }
    }
