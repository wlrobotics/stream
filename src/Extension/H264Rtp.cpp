...
void H264RtpEncoder::inputFrame(const Frame::Ptr &frame) {
    ...
    const int nal_type = H264_TYPE(ptr[0]);
    ...
    if (len > payload_size + 1) {
        ...
        //将sps、pps、非自定义sei这3种nalu也放入gop中
        bool gop_pos = false;
        if(nal_type == H264Frame::NAL_SPS || nal_type == H264Frame::NAL_PPS || nal_type == H264Frame::NAL_IDR) {
            gop_pos = true;
        } else if (nal_type == H264Frame::NAL_SEI) {
            if(pre_nal_type_ == H264Frame::NAL_SPS || pre_nal_type_ == H264Frame::NAL_PPS) {
                gop_pos = true;
            }
        }
        makeH264Rtp(ptr, len, false, gop_pos, pts);
    }
