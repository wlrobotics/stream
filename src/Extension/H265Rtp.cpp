...
void H265RtpEncoder::inputFrame(const Frame::Ptr &frame) {
    ...
    const auto nal_type = H265_TYPE(ptr[0]); //获取NALU的5bit 帧类型
    ...
    if (len > payload_size + 2) {
        ...
        if(frame->keyFrame() || frame->configFrame()) {
            gop_pos = true;
        } else if (nal_type == H265Frame::NAL_SEI_PREFIX) {
            if(pre_nal_type_ == H265Frame::NAL_VPS || pre_nal_type_ == H265Frame::NAL_SPS || pre_nal_type_ == H265Frame::NAL_PPS) {
                gop_pos = true;
            }
        }

        makeH265Rtp(nal_type, ptr, len, false, gop_pos, pts);
    }

    pre_nal_type_ = nal_type;
}

void H265RtpEncoder::makeH265Rtp(int nal_type,const void* data, unsigned int len, bool mark, bool gop_pos, uint32_t uiStamp) {
