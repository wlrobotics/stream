...
public:
    ...
    FrameCacheAble(const Frame::Ptr &frame){
        ...
        raw_sei_payload_ = frame->raw_sei_payload_;
    }
...
TrackType getTrackType(CodecId codecId){
    switch (codecId){
        ...
        case CodecAAC:
        case CodecG711A:
        case CodecG711U:return TrackAudio;
...
void Frame::set_ntp_stamp() {
    if(ntp_time_stamp_ == 0u) {
        ntp_time_stamp_ = toolkit::getCurrentMillisecond(true);
    }
}

std::uint64_t Frame::get_ntp_stamp() {
    return ntp_time_stamp_;
}
