...
public:
    ...
    virtual void reset_pts(std::uint32_t pts)  = 0;
...
private:
    std::uint64_t ntp_time_stamp_ = 0u;
};
...
public:
    ...
    void reset_pts(std::uint32_t pts)  override {
        _pts = pts;
        _dts = _pts;
    }
...
public:
     ...
     */
    FrameWrapper(const Buffer::Ptr &buf,
                     int64_t dts,
                     int64_t pts,
                     int prefix,
                     int offset) : Parent(buf->data() + offset,
                                          buf->size() - offset,
                                          dts,
                                          pts,
                                          prefix){
