...
public:
    ...
    TcpSession::Ptr operator()(const Parser &header,
                                  const HttpSession &parent,
...
template<typename Creator,
         typename HttpSessionType = HttpSession,
         WebSocketHeader::Type DataType = WebSocketHeader::TEXT>
...
protected:
    ...
    bool onWebSocketConnect(const Parser &header) override{
        ...
        weak_ptr<WebSocketSessionBase> weakSelf = 
            dynamic_pointer_cast<WebSocketSessionBase>(HttpSessionType::shared_from_this());
     ...
     * 收到websocket数据包负载
     */
    void onWebSocketDecodePayload(const WebSocketHeader &packet,
                                          const uint8_t *ptr,
                                          uint64_t len,
                                          uint64_t recved) override {
        ...
        switch (header._opcode){
            ...
            case WebSocketHeader::BINARY:{
                ...
                if (_payload_cache.empty()) {
                    //这个包是唯一个分片
                    _session->onRecv(std::make_shared<WebSocketBuffer>(header._opcode,
                                                                       header._fin,
                                                                       std::move(_payload_section)));
                    break;
                }
...
template<typename TcpSessionType,
         typename HttpSessionType = HttpSession,
         WebSocketHeader::Type DataType = WebSocketHeader::TEXT>
class WebSocketSession : public WebSocketSessionBase<TcpSessionCreator<TcpSessionType>,
                                                          HttpSessionType,
                                                          DataType>{
public:
    WebSocketSession(const Socket::Ptr &pSock) : WebSocketSessionBase<TcpSessionCreator<TcpSessionType>,
                                                                           HttpSessionType,
                                                                           DataType>(pSock){}
